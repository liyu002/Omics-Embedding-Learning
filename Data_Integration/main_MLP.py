...
from MLP_network import omicsAdd
# from MLP_network_multi import omicsAdd
...
def main(
    ...
    # Feature addition
    integrated_mRNA, integrated_miRNA = omicsAdd(
        data["mRNA_data"], data["miRNA_data"], data["adj_matrix"], data["labels"], device, dataset_name
    )
    save_pt = dict()
    ...
    torch.save(save_pt, f'./MLP_save_pt_{dataset_name}.pt')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", "--dataset-name", type=str, help="Name of the dataset", default="STAD",
    )
    parser.add_argument(
        "-s1",
        "--m-select",
