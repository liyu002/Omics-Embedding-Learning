...
class ADataset(BasicDataset):
    """
    A dataset class for mRNA expression dataset.
    ...
    def __init__(self, param):
        ...
        # A_df = data_original['mRNA_data']
        A_df = pd.DataFrame(data_save["integrated_mRNA"], index=data_save["data"]["sample_names"], columns=data_save["data"]["mRNA_names"]).T
        # A_df = pd.DataFrame(data_save["data"]["mRNA_data"], index=data_save["data"]["sample_names"],
        #                     columns=data_save["data"]["mRNA_names"]).T
        # Get the sample list
